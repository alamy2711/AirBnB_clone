...
class Test_FileStorage(unittest.TestCase):
    ...
    def test_all_arg(self):
        """
        Test that calling models.storage.all() with an argument raises a TypeError
        """
        with self.assertRaises(TypeError):
            models.storage.all(None)
    
    # Test if new method adds instances to the storage and checks if they are saved
    def test_new(self):
        ...
        self.assertIn("BaseModel." + Based_model_obj.id, models.storage.all().keys())
